...
// In-Memory store for robust fallback when PostgreSQL is offline
const inMemoryUsers = [];
...
export const authService = {
  ...
  async updateProfile(userId, { fullName, avatarUrl, phone, bio }) {
    ...
    if (userIndex !== -1) {
      if (fullName) inMemoryUsers[userIndex].full_name = fullName;
      if (avatarUrl) inMemoryUsers[userIndex].avatar_url = avatarUrl;
      if (phone) inMemoryUsers[userIndex].phone = phone;
      if (bio) inMemoryUsers[userIndex].bio = bio;
      const { password_hash, ...safeUser } = inMemoryUsers[userIndex];
      return safeUser;
    }
    return null;
  },

  // Seed user into memory
  seedUser(user) {
    const idx = inMemoryUsers.findIndex((u) => u.email.toLowerCase() === user.email.toLowerCase());
    if (idx !== -1) {
